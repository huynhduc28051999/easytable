    ...
    goCodepen() {
      ...
      const resourcesTpl
                = '<scr'
                  ...
                  + 'ipt>'
                  + '\n<scr'
                  + `ipt src="//unpkg.com/@vue-table-easy/vue@${version}/libs/umd/vue-table-easy-vue.js"></scr`
                  + 'ipt>'
      ...
      const cssTpl = `@import url("//unpkg.com/@vue-table-easy/vue@${version}/libs/theme-default/index.css");\n${(
        exampleStyle || ''
      ).trim()}\n`
      jsTpl = jsTpl
        ? `${jsTpl
        }\nVue.createApp(Main).use(vue-table-easyVue.useVeTable()).mount(\'#app\')`
        : 'Vue.createApp().use(vue-table-easyVue.useVeTable()).mount(\'#app\')'
      const data = {
        js: jsTpl,
        css: cssTpl,
        html: htmlTpl,
